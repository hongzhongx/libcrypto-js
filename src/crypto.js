...
(function(root, factory) {
  if (typeof exports === 'object' && typeof exports.nodeName !== 'string') {
    ...
    // Browser globals
    root.taiyi = root.taiyi || {};
    factory((root.taiyi.crypto = {}));
  }
  ...
  function PrivateKey(priv, pub) {
    ...
    this.sign = function(hash) {
      return fromBits(sjcl.codec.taiyi.signRecoverably(priv, toBits(hash)));
    };
  }

  PrivateKey.from = function(wif, header) {
    return new PrivateKey(
      sjcl.codec.taiyi.deserializePrivateKey(wif, header)
    );
  };
  ...
  PublicKey.from = function(str) {
    return new PublicKey(sjcl.codec.taiyi.deserializePublicKey(str));
  };

  PublicKey.recover = function(hash, sig) {
    return new PublicKey(
      sjcl.codec.taiyi.recoverPublicKey(toBits(hash), toBits(sig))
    );
  };

  PublicKey.prototype = {
    toString: function() {
      return sjcl.codec.taiyi.serializePublicKey(this._p);
    },
  ...
  function serializePair(k) {
    return {
      private: sjcl.codec.taiyi.serializePrivateKey(k.sec),
      public: sjcl.codec.taiyi.serializePublicKey(k.pub)
    };
  }
