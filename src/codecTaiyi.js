...
sjcl.codec.taiyi = {
  ...
  keysFromPassword: function(account, password) {
    ...
    for (var i = 0; i < sjcl.codec.taiyi.ROLES.length; i++) {
      var role = sjcl.codec.taiyi.ROLES[i];
  ...
  signRecoverably: function(sec, hash, paranoia, fixedKForTesting) {
     ...
     * its X coordinate).
     *
     * note that in the Taiyi blockchain, all public keys are canonically compressed and therefore the
  ...
  recoverPublicKey: function(hash, sig) {
    ...
    for (var j = 0; j <= 1; j++) {
      ...
      var y = sjcl.codec.taiyi._yFromX(x, hasOddParity);
  ...
  serializePublicKey: function(key, net) {
    net = net || sjcl.codec.taiyi.MAINNET;
  ...
  deserializePublicKey: function(pubKey, net) {
    ...
    var y = sjcl.codec.taiyi._yFromX(x, isOdd);
  ...
  deserializePrivateKey: function(wif, header) {
    header = header || sjcl.codec.taiyi.MAINNET.privHeader;
    var curve = sjcl.ecc.curves.k256;
  ...
  _yFromX: function(x, shouldBeOdd) {
    var CURVE = sjcl.ecc.curves.k256;
    var PIDENT = sjcl.codec.taiyi._getPident();
  ...
  _getPident: function() {
    if (!sjcl.codec.taiyi.PIDENT) {
      sjcl.codec.taiyi.PIDENT = sjcl.ecc.curves.k256.field.modulus
        .add(1)
        .normalize()
        .halveM()
        .halveM()
    ...
    return sjcl.codec.taiyi.PIDENT;
  }
};
